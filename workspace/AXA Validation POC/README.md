...
├── Validation Rules.md                # 校验规则文档（直接在此维护，无需上传知识区）
...
- **校验引擎**：Copilot Studio AI Agent（规则直接在 `Validation Rules.md` 中维护）
...
| 文件 | 用途 |
|------|------|
| [`Validation Rules.md`](./Validation%20Rules.md) | 全部校验规则（中英双语）。规则如有变更，直接更新此文件并提交至本分支，无需上传至 Copilot Studio 知识模块。 |
| [`Copilot-Studio-AI-Instructions.md`](./Copilot-Studio-AI-Instructions.md) | Copilot Studio AI Agent 的 Instructions 区参考模板，包含输入输出 JSON Schema。 |
...
4. 规则如有变更：直接更新 `Validation Rules.md` → 提交到本分支，无需上传至 Copilot Studio 知识模块。
