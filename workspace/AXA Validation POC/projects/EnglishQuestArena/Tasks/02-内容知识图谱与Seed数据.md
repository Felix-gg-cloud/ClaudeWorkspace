...
## 状态：⬜ 未开始 | 前置条件：阶段三 Phase 3-A (数据库就绪) 完成后进行

> **调整说明**: 原计划先做阶段二再做三。现改为阶段三先行建库建表，
> 然后阶段二直接往数据库灌高质量 Seed 数据，避免先写 JSON 再迁移的重复劳动。
> 阶段二可与阶段三 Phase 3-C/D/E 并行推进。
...
1. 后端表结构确定后，Seed 数据可以直接灌库，避免先写 JSON 再迁移格式
2. 前端已有 CH1(40 词) + CH2(22 词) + CH3(22 词) = 84 词，足够后端开发调试
3. Auto-Drill 自动补题依赖后端实现，先做后端才能验证 Seed+Auto 的配合效果
4. 阶段二可与阶段三 Phase 3-C/D/E 并行，不阻塞进度
